...
Для того чтобы настроить проект, выполните следующие шаги:
...
Или убедитесь, что у вас установлены следующие библиотеки:
   - `pandas`
   - `numpy`
   - `scikit-learn`
   - `matplotlib`
   ...
   - `PyYAML`
   - `scipy`
и другие, если они указаны в `requirements.txt`.


## Запуск скрипта

Настройте конфигурационный файл `config.yaml`, указав желаемые параметры. 

**Вариант 1** - запуск через CLI. 

После настройки `config.yaml` для запуска основного пайплайна в командной строке необходимо выполнить команду:
...
**Вариант 2** - юпитер блокнот. Выполните все ячейки в `workflow.py`. Изменять параметры можно как в `config.yaml`, так и вручную.
